...
@onready var player1_score_label = $"UI-Elements/Player1Score"
@onready var player2_score_label = $"UI-Elements/Player2Score"
@onready var player1_roll_button = $"UI-Elements/Player1RollButton"
@onready var player2_roll_button = $"UI-Elements/Player2RollButton"
...
@onready var player1_dice_grid = $Board/DiceGrid
@onready var player2_dice_grid = $Board/DiceGrid2
@onready var dice_spawn_position = $Board/DiceSpawnPosition
...
func computer_turn() -> void:
	...
	var tile = player2_dice_grid.get_tile(choice)
	place_dice(tile)


func remove_opponent_dice(row_index, roll_value):
	
	var opponent_rows = player2_rows if player_turn == 1 else player1_rows
	var current_board = player2_dice_grid if player_turn == 1 else player1_dice_grid
	var column = 0
	...
	for value in opponent_rows[row_index]:
		...
		if value == roll_value:
			...
			var tile = current_board.get_tile(destroyed_dice_index)
			print(tile.index)
			var dice = tile.dice
			print(dice)
...
func check_game_over():
	var player1_full = true
	for i in player1_rows:
		if 0 in i:
			player1_full = false
	
	var player2_full = true
	for i in player2_rows:
		if 0 in i:
			player2_full = false
	
	if player1_full or player2_full:
		var player1_scores = calculate_column_score(player1_rows[0]) + calculate_column_score(player1_rows[1]) + calculate_column_score(player1_rows[2])
		var player2_scores = calculate_column_score(player2_rows[0]) + calculate_column_score(player2_rows[1]) + calculate_column_score(player2_rows[2])
...
func update_ui():
	var player1_score = (
			calculate_column_score(player1_rows[0]) 
			+ calculate_column_score(player1_rows[1]) 
			+ calculate_column_score(player1_rows[2])
	)
	
	var player2_score = (
			calculate_column_score(player2_rows[0]) 
			+ calculate_column_score(player2_rows[1]) 
			+ calculate_column_score(player2_rows[2])
	)
